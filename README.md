...
- If a client sends `X-Request-Id`, the same value is preserved in the response.
- If a client omits `X-Request-Id`, the service generates one.
- Request metrics are exposed at `GET /metrics` in Prometheus text format.
- Normal request metrics exclude traffic generated by scraping `/metrics`.

Example:

```bash
curl -i "http://localhost:8181/api/v1/to-do-items?page=1&page_size=10"
curl "http://localhost:8181/metrics"
```

Basic operational checks:

- Use `X-Request-Id` from responses to correlate request traces.
- Watch `http_requests_total` for route traffic volume.
- Watch `http_request_duration_seconds` for request latency.
- Watch `http_request_errors_total` for failing requests.
